...
describe("Family Controller Unit Tests", () => {
  ...
  beforeEach(() => {
    ...
    sequelize.query.mockImplementation((query, options) => {
      if (
        query.includes('FROM "Families" f') &&
        options.replacements &&
        options.replacements.userId
      ) {
...
describe("getUserFamilies", () => {
  it("should get all families for the user", async () => {
    ...
    // Set up the expected data to return from the query
    const mockFamilies = [
      {
        id: "family-123",
        name: "Test Family 1",
    ...
    // Ensure the query returns our mock data
...
describe("deleteFamily", () => {
  ...
  it("should return 403 if user is not the creator", async () => {
    ...
    const req = {
      params: { id: "family-123" },
      user: { id: "user-123" }, // Not the creator
    };
