...
// Create a fixed test UUID
const mockUserUUID = '12345678-1234-1234-1234-123456789012';

// Mock bcrypt to make password comparison always succeed for our test
jest.mock('bcryptjs', () => ({
  compare: jest.fn().mockImplementation((password, hash) => {
    // For our special test user, always return true
    if (hash === 'hashed_special_password') {
      return Promise.resolve(true);
    }
    // For other cases, use normal behavior (which you might need to mock differently)
    return Promise.resolve(false);
  }),
  hash: jest.fn().mockImplementation((password) => {
    if (password === 'test123') {
      return Promise.resolve('hashed_special_password');
    }
...
describe('User API Endpoints', () => {
    ...
    try {
      ...
      try {
        ...
        await User.create({
          ...
          passwordHash: 'hashed_special_password', // This matches our bcrypt mock
          role: 'user'
        }, { transaction });
        ...
        // Add additional mock for the User.findOne to help with login test
        // This is special handling just for the "special@example.com" login test
  ...
  afterAll(async () => {
    try {
      ...
      await User.destroy({
        where: {
          id: [testUser.id, adminUser.id, mockUserUUID]
        },
        force: true
      });
  ...
  describe('POST /api/users/login', () => {
    it('should login successfully with correct credentials', async () => {
      // Special handling for this test to ensure it works with our middleware mock
      // Mock jwt.sign specifically for this test
