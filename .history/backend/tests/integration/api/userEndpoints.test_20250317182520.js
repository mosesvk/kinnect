...
// Mock the JWT token generation - but define TEST_USER_UUID inside the mock's scope
jest.mock('../../../src/middleware/auth', () => {
  // Define the UUID inside the mock factory to avoid reference errors
  const MOCK_USER_UUID = uuidv4();
  
  return {
    protect: jest.fn((req, res, next) => {
      // Use UUID defined within this scope
      req.user = {
        id: MOCK_USER_UUID,
        role: 'user'
      };
      ...
      // Export the UUID for tests to use
      module.exports.TEST_USER_UUID = MOCK_USER_UUID;
    }),
    admin: jest.fn((req, res, next) => {
      next();
    }),
    // Export the UUID for tests to use
    TEST_USER_UUID: MOCK_USER_UUID
  };
});

// Import the auth middleware to access the TEST_USER_UUID
  ...
  beforeAll(async () => {
    try {
      ...
      try {
        ...
        await User.create({
          id: TEST_USER_UUID, // Use the UUID from the imported middleware mock
          firstName: 'Special',
          lastName: 'TestUser',
