...
describe("User Validations Integration Tests", () => {
  ...
  describe("User Deletion Validation", () => {
    // Update the initial test app creation to include the deletion validation
    const updatedTestApp = createValidationTestApp({
      "user-registration": validateUserRegistration,
      "user-login": validateUserLogin,
      "user-update": validateUserUpdate,
      "user-deletion": validateUserDeletion,
    });

    it("should pass validation with password provided", async () => {
      const validData = {
        password: "correctPassword123",
      };

      const response = await request(updatedTestApp)
        .post("/test/user-deletion")
        .send(validData);
