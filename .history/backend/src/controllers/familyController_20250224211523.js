...
const familyController = {
  ...
  joinFamily: async (req, res) => {
    try {
      ...
      // Find the family with the given invitation code
      const family = await Family.findOne({ invitationCode: invitationCode });

      if (!family) {
        return res.status(404).json({
          success: false,
          message: "Invalid invitation code or family not found",
        });
      }
      ...
      // Add user to family with default member role
      family.members.push({
        userId,
        role: "member",
