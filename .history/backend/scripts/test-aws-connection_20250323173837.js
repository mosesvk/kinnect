...
fs.writeFileSync(
  testFilePath,
  "This is a test file for AWS S3 connection testing."
);
...
async function testAwsConnection() {
  ...
  try {
    ...
    console.log(
      `\nIMPORTANT: The test file "${testFile.Key}" was left in your S3 bucket.`
    );
    console.log("You can verify it in the AWS console and delete it manually.");
    console.log(
      `To find it, go to: https://s3.console.aws.amazon.com/s3/buckets/${testFile.Bucket}?region=${process.env.AWS_REGION}&prefix=test-uploads/`
    );
  } catch (error) {
    ...
    if (error.name === "CredentialsProviderError") {
      ...
      console.error(
        "- Verify your IAM user has the required permissions for this bucket"
      );
